...
The default runtime image (`ghcr.io/earayu/treadstone-sandbox`, now built on the validated reconstructed sandbox base) still inherits the same opaque bootstrap behavior and is **not rootless-compatible**. Its startup scripts create the `gem` user/group and perform other bootstrap steps as root on every start. Because of that, Treadstone does **not** force the main sandbox container to run as UID/GID 1000.
