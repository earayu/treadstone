...
`202` 而非 `201`，因为资源尚未真正创建完成（K8s Pod 还在启动），`202` 准确表达"请求已接受，正在异步处理"。
...
| 运行时状态 | **K8s CRD 为 Source of Truth，DB 为派生的查询视图** | status, endpoints。由 K8s Watch 事件驱动写入 DB |
| K8s 关联与同步元数据 | **DB 存映射** | k8s_sandbox_name, k8s_namespace, k8s_resource_version, last_synced_at |

**核心原则：**
- **K8s CRD 是运行时状态的 Canonical Source of Truth**，DB 中的 status/endpoints 是从 CRD 派生的查询视图
- **所有读查询只走 DB**，不查 K8s API Server。K8s 状态通过 Watch 事件实时同步到 DB
- **写操作采用异步语义**：API 立即返回（202/204），不等待 K8s 真正完成
...
class Sandbox(Base):
    ...
    k8s_resource_version: str | None  # 上次同步时的 K8s resourceVersion，用于对账时判断是否过期
    last_synced_at: datetime | None   # 上次成功同步时间，用于检测 Watch 失效

    # 运行时状态（K8s CRD 为 Source of Truth，DB 为派生的查询视图）
...
K8s Watch 事件到达
    ...
    │       校验状态转换合法性 → 更新 DB（乐观锁）
    │       同时更新 k8s_resource_version 和 last_synced_at
    │
...
3. 逐条比对：
   - DB 有、K8s 有 → 比对 `k8s_resource_version`，相同则跳过（无变化）；不同则比对 status，修正 DB（乐观锁），更新 `k8s_resource_version` 和 `last_synced_at`
...
- K8s 与业务数据库状态同步行业最佳实践研究报告 — 基于 QuestDB Cloud、Crossplane、KubeBlocks 等真实生产系统的架构分析
