...
### About autogenerated names

When `name` is omitted, Treadstone generates a human‑readable label like `sb-lively-otter-3f9a` that meets the
same validation rules as custom names (lowercase letters, digits, hyphens, 1–55 chars; start/end alphanumeric).
Names only need to be unique per account. Use the sandbox `id` for APIs; treat the name as a display label.
