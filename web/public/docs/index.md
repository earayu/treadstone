...
You only need those two names and what they stand for at first. When you want concrete URLs and field names (**Web**, **MCP**, **Proxy**, and **`urls.*`** in API responses), read [Sandbox endpoints](/docs/sandbox-endpoints.md). For REST shape and auth, continue with [REST API Guide](/docs/rest-api-guide.md) and [API Keys & Auth](/docs/api-keys-auth.md).
