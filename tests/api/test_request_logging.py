...
@pytest.mark.asyncio
async def test_health_generates_request_id_without_structured_log(request_log_stream):
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.get("/health")

    assert response.status_code == 200
    assert response.headers["X-Request-Id"]
    assert request_log_stream.getvalue().strip() == ""


@pytest.mark.asyncio
async def test_request_id_passthrough_on_health_without_structured_log(request_log_stream):
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
...
@pytest.mark.asyncio
async def test_unknown_path_still_logs_json(request_log_stream):
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.get("/no-such-route-for-logging-test")

    assert response.status_code == 404
    payload = json.loads(request_log_stream.getvalue().strip().splitlines()[-1])
    assert payload["event"] == "http_request"
    assert payload["path"] == "/no-such-route-for-logging-test"
